...
def read_raster(raster_path):
    ...
    with rio.open(raster_path) as src:
        ...
        meta = src.meta.copy()
        print(
            "Number of active cells (non-masked) of raster ",
            raster_path,
            ": ",
            np.ma.count(raster_np),
        )
...
class RasterDataPlotter:
    ...
    def make_hist(
        self, legendx, legendy, fontsize, output_file, figsize, set_ylim=None, set_xlim=None
    ):
        ...
        plt.text(raster_np.mean() * 0.70, max_ylim * 0.9, f"Sfuzzy: {raster_np.mean():.4f}")

        # Save fig
        plt.savefig(output_file, dpi=300)
        plt.clf()

    def plot_continuous_w_window(
        self, output_file, xy, width, height, bounds, cmap=None, list_colors=None
    ):
