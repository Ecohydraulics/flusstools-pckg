...
class LasPoint:
    ...
    def create_dem(self, target_file_name="", pixel_size=1.0, **kwargs):
        ...
        default_keys = {
            "src_shp_file_name": self.shapefile_name,
        ...
        if not os.path.isfile(default_keys["src_shp_file_name"]):
            logging.info(
                ...
                % default_keys["src_shp_file_name"]
            )
    ...
    def export2shp(self, **kwargs):
        ...
        if kwargs.get("shapefile_name"):
            ...
            self.shapefile_name = os.path.abspath("") + f"/{self.las_file.filename}.shp"
        ...
        logging.info(
            " * Writing geopandas.GeoDataFrame to shapefile (%s) ..." % self.shapefile_name
        )
    ...
    def _build_data_frame(self):
        ...
        self.gdf = geopandas.GeoDataFrame(pd.DataFrame(point_dict), crs="EPSG:%04i" % self.epsg)
        logging.info("   -- Done.")
    ...
    def _get_xyz_array(self):
        ...
        dem_array = np.empty((3, len(pts)), dtype=float)
    ...
    def _parse_attributes(self):
        ...
        for attr in self.attributes:
            try:
                ...
                logging.error(
                    "Non-existing attribute %s. Valid attributes are: %s"
                    % (str(attr), dict2str(wattr))
                )
            except KeyError:
                logging.error(
                    "Non-existing las-file key %s - valid are: " % str(attr)
                    + ", ".join(dir(self.las_file))
                )
        return point_dict
