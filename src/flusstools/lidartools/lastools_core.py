...
def ar1_acorr(series, maxlags=""):
    ...
    lower_band, upper_band = zip(
        *[r_confidence_interval(phi**k, n - k) for k in lags], strict=False
    )
    ...
    for k in lags:
        if k == 0:
            ...
            numerator = (
                1.0
                / (n - k)
                * sum([(x1 - xbar) * (x2 - xbar) for x1, x2 in zip(s1, s2, strict=False)])
            )
...
def lof_text(pwd, src):
    ...
    if type(src) == str:
        f.writelines("%s\n" % i for i in get_las_files(src))
    else:
        # this is the case when there are multiple source folders
        f.writelines("%s\n" % i for i in [name for source in src for name in get_las_files(source)])
    f.close()
    return filename
...
def process_lidar(
    ...
    for class_type in classes:
        ...
        cmd(
            "%slas2las.exe -lof %s -cores %i -keep_classification %i -odir %s -olas"
    ...
    cmd(
        "%slastile.exe -lof %s -cores %i -o tile.las -tile_size %i -buffer 5 -faf -odir %s -olas"
    ...
    cmd(
        "%slasclassify.exe -lof %s -cores %i %s -odir %s -olas"
    ...
    cmd(
        "%slastile.exe -lof %s -cores %i -remove_buffer -odir %s -olas"
    ...
    if ground_poly != "":
        ...
        cmd(
            "%slasclip.exe -lof %s -cores %i -poly %s -interior -donuts -odir %s -olas"
        ...
        cmd(
            "%slasclip.exe -lof %s -cores %i -poly %s -donuts -odir %s -olas"
            % (lastoolsdir, lof, cores, ground_poly, odir)
        )
    ...
    if keep_orig_pts:
        ...
        if ground_poly != "":
            ...
            sources = [
                lidardir + "06_separated" + "/" + "02-Ground" + "/",
                lidardir + "00_separated" + "/" + "02-Ground" + "/",
            ]
    # just use new points
    elif ground_poly != "":
    ...
    if keep_orig_pts or (ground_poly != ""):
        ...
        cmd(
            "%slastile.exe -lof %s -cores %i -o tile.las -tile_size %i -faf -odir %s -olas"
            % (lastoolsdir, lof, cores, tile_size, odir)
        )
    ...
    if keep_orig_pts:
        ...
        cmd(
            "%slasduplicate.exe -lof %s -cores %i -lowest_z -odir %s -olas"
            % (lastoolsdir, lof, cores, odir)
        )
    ...
    if ground_poly != "":
        ...
        sources = [
            lidardir + "06a_separated_coarse" + "/" + "05-Vegetation" + "/",
...
def white_noise_acf_ci(series, maxlags=""):
    ...
    lower_lims, upper_lims = list(zip(*lims, strict=False))
