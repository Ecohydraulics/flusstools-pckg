...
def plot_funs(dc_param_range, dc_fuzzy_funs, dc_disfuzzy_funs):
    ...
    ax2.plot(
        dc_param_range["po"] / 100, dc_fuzzy_funs["po_nc"], "gray", linewidth=1.5, label="High"
    )
    ax2.set_xlabel("n [-]")
    ax2.set_ylabel("\u03bc [-]")
    ax2.legend()
    ...
    ax4.plot(
        dc_param_range["ratio"],
        dc_fuzzy_funs["ratio_c"],
        "gray",
        linestyle="--",
        linewidth=1.5,
        label="Bridging",
...
def compute_bcs(dc_limits):
    ...
    for fun in parameter_order:
        ...
        dc_b.update({f"{fun}_b": b})
        dc_c.update({f"{fun}_c": c})
...
def activate_fuzzy_funs(probe, dc_param_range, dc_fuzzy_funs):
    ...
    for k, par in enumerate(parameters):
        ...
        low_level = fuzz.interp_membership(
            dc_param_range[par], dc_fuzzy_funs[f"{par}_{low}"], probe[k]
        )
        high_level = fuzz.interp_membership(
            dc_param_range[par], dc_fuzzy_funs[f"{par}_{high}"], probe[k]
        )
        dc_fuzzy_activated.update({f"{par}_{low}_low": low_level, f"{par}_{high}_high": high_level})
...
def plot_aggregation(
    ...
    ax0.fill_between(
        dc_param_range["doc"], x0, dc_mu_desfuzzy["mu_mc"], facecolor="gainsboro", alpha=0.7
    )
    ax0.plot(
        dc_param_range["doc"],
    ...
    ax0.fill_between(
        dc_param_range["doc"], x0, dc_mu_desfuzzy["mu_sc"], facecolor="black", alpha=0.7
    )
    ax0.plot(
        dc_param_range["doc"],
    ...
    ax1.plot(
        [degree_of_clogging, degree_of_clogging],
        [0, 0.7 * activation],
        "k",
        linewidth=1.5,
        alpha=0.9,
...
def apply_fuzzy_rules(dc_af=None, dc_desfuzzy_funs=None, centroid=False, mu_list=None):
    ...
    if centroid:
        ...
        mu_sc_value = max(
            [
                np.fmin(dc_af["kf_c_low"], dc_af["fs_c_high"]),
        ...
        mu_mc_value = max(
            [
                np.fmin(dc_af["kf_c_low"], dc_af["idoc_nc_high"]),
        ...
        mu_nc_value = max(
            [
                np.fmin(dc_af["idoc_nc_high"], dc_af["kf_nc_high"]),
    ...
    dc_mu_defuzzy_values = {"mu_sc": mu_sc_value, "mu_mc": mu_mc_value, "mu_nc": mu_nc_value}
...
def find_centroids(dc_desfuzzy_funs, dc_param_range):
    ...
    for k, mu_list in enumerate(mu_lists):
        dc_mu_desfuzzy, _ = apply_fuzzy_rules(None, dc_desfuzzy_funs, True, mu_list)
        aggregated = np.fmax(
        ...
        if k == 0:
            ...
            name = "sc"
        dic_defuzzy_centroids.update({f"{name}_centroid": centroid})
...
def add_columns(df_samples):
    ...
    columns = [
        "dc_c",
        "dc",  # degree of clogging corrected and initial
        "mu_nc",
        "mu_mc",
        "mu_sc",  # membership of defuzzified functions
        "mu_fsf_low",
        "mu_fsf_high",  # membership of fuzzy functions
        "mu_kf_low",
        "mu_kf_high",
        "mu_n_low",
        "mu_n_high",
...
def add_results(
    df, step, degree_of_clogging_corrected, degree_of_clogging, dc_mu_desfuzzy_values, dc_af
):
    ...
    df.loc[step, ["mu_nc", "mu_mc", "mu_sc"]] = [
        dc_mu_desfuzzy_values["mu_nc"],
