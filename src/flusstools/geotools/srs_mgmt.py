...
def reproject_raster(source_dataset, source_srs, target_srs):
    ...
    (max_x, min_y, new_z) = coord_trans.TransformPoint(
        src_geo_transform[0] + src_geo_transform[1] * x_size,
    ...
    tar_file_name = (
        src_file_name.split(".tif")[0] + "_epsg" + target_srs.GetAuthorityCode(None) + ".tif"
    )
    create_raster(
        tar_file_name,
        raster_array=tar_dataset.ReadAsArray(),
...
def reproject_shapefile(source_dataset, source_layer, source_srs, target_srs):
    ...
    tar_file_name = (
        verify_shp_name(source_dataset.GetName(), shorten_to=4).split(".shp")[0]
        + "_epsg"
        + target_srs.GetAuthorityCode(None)
        + ".shp"
    )
    ...
    for i in range(src_lyr_def.GetFieldCount()):
    ...
    while feature:
        ...
        for i in range(tar_lyr_def.GetFieldCount()):
